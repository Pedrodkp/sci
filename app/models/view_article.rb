class ViewArticle < ActiveRecord::Base
  def self.search(search_by_text, search_by_id, search_by_date_ini, search_by_date_fim, sort_column, sort_direction)
    ...
    f = f.where([(['   view_articles.title LIKE ? collate utf8_general_ci '+
                   'OR view_articles.body LIKE ? collate utf8_general_ci '+
                   'OR exists (select 1 from relationships r, taxonomies t where r.article_id = view_articles.id and t.id = r.taxonomy_id and t.code LIKE ? collate utf8_general_ci)'] * 
    ...
    #f = f.group("view_articles.id, view_articles.title, view_articles.body, view_articles.created_at, view_articles.updated_at, view_articles.user_id, view_articles.likes")
